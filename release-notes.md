...
-   The service offers a new customization interface that you can use to specify how it pronounces unusual words that occur in your input. The interface includes a number of new methods that allow you to create and manage custom voice models and the word/translation pairs they contain. You can then use your custom models when synthesizing text to audio. The service lets you specify sounds-like translations or phonetic translations in the standard International Phonetic Alphabet (IPA) representation or the proprietary {{site.data.keyword.IBM_notm}} Symbolic Phonetic Representation (SPR); you use the Speech Synthesis Markup Language (SSML) to specify phonetic translations.
    ...
    The customization interface includes a collection of new HTTP methods that have the names `customizations`, `customizations/{customization_id}`, `customizations/{customization_id}/words`, and `customizations/{customization_id}/words/{word}`. The service also provides a new `pronunciation` method that lets you obtain the pronunciation for any word and a new `voices/{voice}` method that lets you get detailed information about a specific voice. In addition, existing methods of the service's interface have been updated to accept custom voice model parameters as needed.

    For more information about customization and its interface, see [Understanding customization](/docs/services/text-to-speech/custom-intro.html), [Using customization](/docs/services/text-to-speech/custom-using.html), and the [API reference ![External link icon](../../icons/launch-glyph.svg "External link icon")](https://www.ibm.com/watson/developercloud/text-to-speech/api/v1/){: new_window}.

    > **Note:** The customization interface is a beta release that currently supports US English only. All `customization` methods and the `pronunciation` method can currently be used to create and manipulate custom voice models and word translations only in US English.
...
-   The list of voices supported for synthesized speech has changed and expanded. The service now supports a number of additional voices, languages, and dialects with the `synthesize` methods. For a complete list of supported voices, see [Specifying a voice](/docs/services/text-to-speech/http.html#voices).
...
-   For the `voices` method, the following error codes have changed:
    -   Error code 406 ("Not acceptable. Unsupported MIME type.") has been removed.
    -   Error code 415 ("Unsupported Media Type") has been added.
