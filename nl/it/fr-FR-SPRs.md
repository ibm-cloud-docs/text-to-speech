...
# Simboli per il francese
{: #frSymbols}

Le seguenti sezioni descrivono i simboli validi per il francese.
...
1.  `x` viene eliminato in alcuni contesti.
...
In francese, il simbolo `_` (carattere di sottolineatura) può essere utilizzato dopo la consonante finale di una parola (ma all'interno delle virgolette che racchiudono l'SPR) per indicare che si tratta di una consonante di liaison. Una consonante di liaison viene pronunciata solo se la parola che segue inizia con una vocale.
...
Seguono esempi di parole con e senza il simbolo di liaison:

-   `"p0'oe't1it_"`: la `t` viene pronunciata solo se la parola che segue inizia con una vocale.
-   `"nEt"`: la `t` viene sempre pronunciata.

Una voce di dizionario *petit* con il valore di traduzione `"p'oe't1it_"` ha la `t` finale pronunciata nella stringa di input *un petit ami* ma non nella stringa di input *un petit chien*. Una voce con il valore di traduzione `"nEt"` ha la `t` finale pronunciata a prescindere dal contesto.
