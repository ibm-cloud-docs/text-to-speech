...
# Symbole für Französisch
{: #frSymbols}

In den folgenden Abschnitten sind die gültigen Symbole für Französisch beschrieben.
...
1.  Das `x` wird in bestimmten Kontexten elidiert.
...
In Französisch kann nach einem Konsonanten am Wortende (jedoch innerhalb der doppelten Anführungszeichen, in die die SPR-Schreibweise eingeschlossen ist), mit dem Zeichen `_` (Unterstreichungszeichen) angegeben werden, dass es sich um einen Bindekonsonanten handelt. Ein Bindekonsonant wird nur dann ausgesprochen, wenn das darauffolgende Wort mit einem Vokal beginnt.
...
-   `"p0'oe't1it_"`: Das `t` wird nur dann ausgesprochen, wenn das nächste Wort mit einem Vokal beginnt.
-   `"nEt"`: Das `t` wird immer ausgesprochen.

Bei einem Wörterbucheintrag *petit* mit dem Umsetzungswert `"p'oe't1it_"` wird das abschließende `t` in der Eingabezeichenfolge *un petit ami* ausgesprochen, jedoch nicht in der Eingabezeichenfolge *un petit chien*. Bei einem Eintrag mit dem Umsetzungswert `"nEt"` wird das abschließende `t` unabhängig vom Kontext ausgesprochen.
