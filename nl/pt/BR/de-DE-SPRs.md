...
As seções a seguir descrevem os símbolos válidos para o alemão.
...
As vogais desagradáveis em francês ocorrem principalmente em palavras de empréstimo estrangeiro.
...
1.  O símbolo do IPA <code>&#240;</code> (Unicode do IPA `00F0`) ocorre somente em empréstimos linguísticos de idiomas estrangeiros, principalmente do inglês (por exemplo, *The New York Times*). No entanto, o serviço compreende esse som como o símbolo do IPA `d` (Unicode do IPA `0064`).
1.  O símbolo do IPA <code>&#952;</code> (Unicode do IPA `03B8`) ocorre somente em empréstimos linguísticos de idiomas estrangeiros, principalmente do inglês (por exemplo, *thriller*). No entanto, o serviço realiza esse som como um símbolo de IPA <code>& #678;</code>(IPA Unicode `02A6`).
