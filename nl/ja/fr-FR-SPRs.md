...
以下の各セクションにフランス語の有効な記号をまとめます。

## 母音
{: #frVowels}

<table style="width:90%">
  <caption>表 1. 母音 (フランス語)</caption>
  <tr>
...
1.  `x` は、特定のコンテキストでは省略されます。

## 子音
{: #frConsonants}

<table style="width:90%">
  <caption>表 2. 子音 (フランス語)</caption>
  <tr>
    <th style="width:20%; text-align:center; vertical-align:bottom">
      フランス語<br/>SPR 記号
    </th>
...
フランス語では、単語の最後の子音の後 (ただし、SPR を囲む二重引用符内) に `_` (下線) を使用して、リエゾンの子音であることを示すことができます。 リエゾンの子音が発音されるのは、続く単語が母音で始まる場合のみです。

<table style="width:54%">
  <caption>表 3. リエゾン (フランス語)</caption>
  <tr>
...
-   `"p0'oe't1it_"`: `t` が発音されるのは、続く単語が母音で始まる場合のみです。
-   `"nEt"`: `t` は常に発音されます。

辞書の項目 *petit* では、トランスレーション値は `"p'oe't1it_"` であり、最後の `t` は、入力ストリング *un petit ami* では発音されますが、入力ストリング *un petit chien* では発音されません。 トランスレーション値 `"nEt"` の項目では、最後の `t` はコンテキストに関係なく発音されます。
